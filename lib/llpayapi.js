 ...
 * 取网银支付的html
 */
Webllpay.prototype.getBankPayHtml = function(jsonData, cb) {
  if (!jsonData.risk_item) {
    // 风险参数必须提供用户注册时间:risk_item.user_info_dt_register
    return "<div style='width:100%;color:red;text-align:center;padding-top:20px;'>缺少风险参数:risk_item</div>";
  }
  ...
  jsonData = util.extend(jsonData, {
    version: "1.0",  //版本号
    busi_partner: "101001",  // 业务类型，连连支付根据商户业务为商户开设的业务类型； （101001：虚拟商品销售、109001：实物商品销售、108001：外部账户充值）
    pay_type: "1"
  });
  var keys = [
    'version', 'oid_partner', 'user_id', 'timestamp', 'sign_type', 'busi_partner', 'no_order',
    'dt_order', 'name_goods', 'money_order', 'notify_url', 'pay_type'
  ];
  var optionKeys = ['charset_name', 'risk_item', 'valid_order', 'url_order', 'userreq_ip', 'url_return', 'info_order', 'bank_code']
  jsonData = util.pick(jsonData, keys.concat(optionKeys)); //拾取连连支付所支持的参数
  console.log('最终的jsonData: ', jsonData);
  //除去待签名参数数组中的空值和签名参数
  var para_filter = util.paraFilter(jsonData);
  //对待签名参数数组排序
  var para_sort = util.sortObjectByKey(para_filter);
  //生成签名结果
  var mysign = this._buildRequestMysign(para_sort);

  jsonData.sign_type = this.config['sign_type'].trim().toUpperCase();
  jsonData.sign = mysign;

  // 检测参数是否齐全
  for (var i = 0, len = keys.length ; i < len ; i++) {
    if (!jsonData.hasOwnProperty(keys[i]) || !jsonData[keys[i]]) {
      return "<div style='width:100%;color:red;text-align:center;padding-top:20px;'>参数错误，请检查！</div>";
    }
  }
  var method = "POST";
  var button_name = "3秒内不能正常进入支付页面，请点击此处";
  var tip_name = "正在进入连连安全支付...";
  var sHtml = "<!DOCTYPE html><html><head><meta name='viewport' content='width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no' /><title>正在进入连连安全支付</title></head><body><form style='width:100%;text-align:center;padding-top:50px;' id='llpaysubmit' name='llpaysubmit' action='" + url.pay_url + "' method='" + method + "'>";
  for (var key in jsonData) {
    sHtml += "<input type='hidden' name='" + key + "' value='" + jsonData[key] + "'/>";
  }
  sHtml += "<label style='font-size:14px;' for='submitBtn'>" + tip_name + "</label>";
  sHtml += "<br /><br />";
  //submit按钮控件请不要含有name属性
