 ...
 * @return 提交表单HTML文本
 */
Webllpay.prototype._buildRequestForm = function(para_temp,method,button_name) {
	method = method || "POST";
	button_name = button_name || "正在进入连连安全支付...";
	//待请求参数数组
	...
	sHtml += "<input type='submit' style='border:none;background: none;color: green;' value='" + button_name + "'></form>";
	...
	request.post({url:url.llpay_query_url,body:obj},function(error, response, body){
		if(!error && response.statusCode === 200){
			try{
				...
				var cloneData = util.clone(resultData);
				//有三个字段不参与验证
				delete cloneData.bank_name;
				delete cloneData.memo;
				delete cloneData.card_no;
				if(self.verify(cloneData)){
					cb(null,resultData);
				}else{
					cb('数据被篡改',resultData);
				}
